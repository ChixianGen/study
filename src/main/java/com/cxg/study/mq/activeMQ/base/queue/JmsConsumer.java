package com.cxg.study.mq.activeMQ.base.queue;   // Administrator 于 2019/7/30 创建;
        ...
        consumer.setMessageListener(message -> {
            if (null != message) {
                ...
                if (message instanceof TextMessage) {
                    TextMessage textMessage = (TextMessage) message;
                    try {
                        Optional.of("【消息监听】接收到的【TextMessage】订阅消息：" + textMessage.getText()).ifPresent(System.out::println);
                    } catch (JMSException e) {
                        e.printStackTrace();
                    }
                    ...
                    try {
                        String string = mapMessage.getString("String");
                        System.out.printf("【消息监听】接收到的【MapMessage】订阅消息：String：【%s】\n", string);
                    } catch (JMSException e) {
                        e.printStackTrace();
                    }
                }
